...
class Database:
    ...
    def clear_empty_rows(self):  # functional, non-implemented

        del_rows = []

        for row in range(len(self.temp)):
    ...
    def add_defaults(self):
        ...
        con.commit()
        con.close()
        self.temp.append(tuple([None for i in range(len(self.column_classes))]))
    ...
    def general_query(self, start: int, end: int, search=False, q=False):  # search holds the text to search for. start/end will
        if self.temp is not None and not search and not q:
